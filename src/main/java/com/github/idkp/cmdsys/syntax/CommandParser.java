...
import com.github.idkp.cmdsys.CommandRegistry;
...
public final class CommandParser {
    ...
    public ParsedCommand parse(Sender sender, CommandRegistry registry, String source) {
        ...
        if (command == null) {
            return new FailureResult(ParsedCommand.Type.INVALID_COMMAND, "Unknown command.");
        }
        ...
        if (currentBranch == null) {
            ...
            if (unhandledBranch == null) {
                return new FailureResult(ParsedCommand.Type.MISSING_BRANCH, "A command branch must be present to run this command.");
            } else {
        ...
        if (command.hasPermission() && !sender.hasPermission(command.getPermission())) {
            return new FailureResult(ParsedCommand.Type.INSUFFICIENT_PERMISSIONS, "Insufficient permissions.");
        }
